...
try:
    ...
    while True:
        has_headers = input(
            "Does the first row of the spreadsheet contain headers or the text file name? (Y/N)\n").upper()
        if has_headers == "Y" or has_headers == "N":
            break
...
for col_num in range(1, sheet.max_column+1):
    ...
    file_name = f"text_file_{file_count}.txt" if has_headers == False else sheet.cell(
        row=1, column=col_num).value
    file = open(os.path.join("text_files", file_name), "w")
    for row_num in range(1, sheet.max_row+1):
        if has_headers == True and row_num == 1:
            continue   # skip writing the first line if it is a header
        # get text from cell
        line = sheet.cell(row=row_num, column=col_num).value
        if line != None:
