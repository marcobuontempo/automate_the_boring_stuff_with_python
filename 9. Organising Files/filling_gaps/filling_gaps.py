...
def close_gaps(folder):
    ...
    # initialise numbering system at first number found + 1
    last_num = int(matches[0].group(2)) + 1

    for i in range(1, len(matches)):    # starts at 2nd item (as first item won't be renamed)
        match = matches[i]
        old_file = os.path.join(
            folder, f"{match.group(1)}{match.group(2)}{match.group(3)}")    # old file path

        # get prefix 0's for the numbering (i.e. 001 instead of 1)
        zeroes = "0" * (num_len - len(str(last_num)))
        new_num = zeroes + str(last_num)    # new number

        new_file = os.path.join(
            folder, f"{match.group(1)}{new_num}{match.group(3)}")   # new file path
...
def add_gap(folder, skip_num):
    ...
    for match in matches:
        if int(match.group(2)) >= skip_num:
            ...
            # reverse matches as we will rename files starting with the largest number suffix
            matches.reverse()
...
prefix = input(
    "Enter the file prefix to match (not including the numbers themselves):\n")
file_regex = re.compile(rf"({prefix})(\d+)(\..+)")
# find function to perform
close_or_add = input(
    "Do you want to close gaps, or add a gap? [close | add]\n")
