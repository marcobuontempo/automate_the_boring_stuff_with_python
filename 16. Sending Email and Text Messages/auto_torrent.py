...
# Path to qBittorrent application
QBITTORRENT_PATH = "/Applications/qbittorrent.app"

# Get email settings from user
email_receiver_address = input(
    "Enter the email address to receive emails from:\n")
email_receiver_password = input("Enter the password for the email address:\n")
email_sender_address = input(
    "Enter the email address that will send the torrent links through email (i.e. only emails sent from this selected address will be able to launch qBittorrent):\n")
    ...
    if raw_messages:
        for UID in raw_messages:
            message = pyzmail.PyzMessage.factory(
                raw_messages[UID]["BODY[]"])   # Get message
            if message.get_subject().lower() != "torrent link":
                continue    # Skip if subject is not "Torrent Link"
            link = message.html_part.get_payload().decode(
                message.html_part.charset)    # Get link
...
time_elapsed = 0
while True:
    # Run every 30 minutes from the time of starting program
    if time_elapsed % (60*30) == 0:
        time_elapsed = 0    # Reset time elapsed every 30 minutes
        magnet_links = get_magnet_links()
        for link in magnet_links:
            # Open qBittorrent with each magnet link found
            subprocess.Popen(QBITTORRENT_PATH + " " + link)
